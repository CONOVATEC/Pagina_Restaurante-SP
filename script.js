...
function carouselFunctionality(
  ...
  const autoSlide = () => {
    ...
    const nextIndex =
      (parseInt(activeControl?.dataset.index) + 1) % carouselControls.length;
    carouselItems.style.transform = `translateX(-${
      nextIndex * widthPercentageForEachItem
    }%)`;
    carouselControls.forEach((control) =>
      control.classList.remove(activatedClass)
    );
  ...
  carouselControls.forEach((control) => {
    control.addEventListener('click', (e) => {
      ...
      carouselItems.style.transform = `translateX(-${
        clickedIndex * widthPercentageForEachItem
      }%)`;
